...
var carPayType = (function(){
    if ($('#ptype-house option:selected').text() == "Outright") {
...
var deposit = (function(){
    ...
    $('#upfront-sum').val((($('#initial').val()/100)*$('#house-fullpay').val()).toFixed(2));
    $('#remain-balance').val((+$('#house-fullpay').val() - +$('#upfront-sum').val()).toFixed(2));
});
...
$('.house-pay-form').mouseenter(carPayType).mouseleave(carPayType);
...
$('#monthly-payments').mouseup(function(){
    ...
    $('#contract-sum').val(($('#total-costs').val()/( +$('#clength').val()*12)).toFixed(0));
    $('#interest-sum').val(((((+t.val()+((+t.val() *(+t.val()-1))/2))/+t.val())* +l.val() *(a.val()/100))).toFixed(2));
    $('#total-costs').val(+$('#interest-sum').val() + +$('#house-fullpay').val() + +$('#loan-fee').val());
});
...
// - - - - - - - - - - - --  final choice  - - - - - - - - - - - - - - - - -  -  
    var selected = (function(){
        ...
        if ($('#ptype-car option:selected').text() == "Outright") {
            ...
            $('.needed').text('Initial House Deposit');
